...
describe("Axe API Soft Delete", () => {
  ...
  test("testing general stuffs", async () => {
    // Main data
    const response1 = await axios.post("/customers", DATA);

    // Child data
    const response2 = await axios.post(
      `/customers/${response1.data.id}/children`,
      DATA
    );
    await axios.post(`/customers/${response1.data.id}/children`, {
      name: "Item 2",
    });

    // Soft delete
    const response3 = await axios.delete(
      `/customers/${response1.data.id}/children/${response2.data.id}`
    );
    ...
    const response5 = await axios.get(
      `/customers/${response1.data.id}/children/all`
    );
    expect(response5.data.length).toBe(1);

    // We can not do any action on the delete item
    const deletedItemURL = `/customers/${response1.data.id}/children/${response2.data.id}`;
    ...
    const parentResponse = await axios.get(`/customers?with=children`);
    ...
    const response7 = await axios.delete(
      `/customers/${response1.data.id}/children/${response2.data.id}/force`
    );
  ...
  test("testing child-parent queries", async () => {
    ...
    const response3 = await axios.post(
      `/customers/${response1.data.id}/children/${response2.data.id}/children`,
      DATA
    );

    // Should be able to see undeleted parent data
    let addressResponse = await axios.get(
      `/customers/${response1.data.id}/children/${response2.data.id}/children/${response3.data.id}?with=parent`,
      DATA
    );
    ...
    try {
      await axios.delete(
        `/customers/${response1.data.id}/children/${response2.data.id}/children/${response3.data.id}/force`
      );
