...
This hook/event is called the after fetching search results.

It can be used in the `SEARCG` handler.

::: code-group

```ts [app/v1/Hooks/User/onAfterSearch.ts]
import { IAfterSearchContext } from "axe-api";

export default async (parameters: IAfterSearchContext) => {
